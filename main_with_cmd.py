...
class App(Tk):
    def __init__(self):
        ...
        for i in range(9):
            for j in range(9):
                if self.board[i][j]==0:
                    ...
                    bg="#967c5f", fg="white", font="Helvetica 15 bold")
                    self.label.place(relheight=1/3, relwidth=1/3, relx=i%3/3, rely=j%3/3)
                    self.label.bind("<ButtonRelease-1>",lambda event, pos=82: self.select_cell(pos))
                    self.my_entries.append(self.label)
        #  with button and labels
        ...
        self.check_button = Button(self.button_frames, text="Sprawdź", command=lambda: self.check(self.my_entries), bg="#b0ebba")
        ...
        self.new_game_label.bind("<ButtonRelease-1>",lambda event, pos=82: self.select_cell(pos))
        # new game buttons
    ...
    def generate_board(self, k):
        ...
        solve(board,0,0)
        self.full_board=np.matrix.copy(board)
        delete_nums(board,k)
        return board
    ...
    # function which checks which cells are correctly filled
    def check(self, my_entries):
    ...
    def new_game(self, board, level):
        if level=="e":
            k=np.random.randint(40,47)
        elif level=="n":
            k=np.random.randint(47,54)
        else:
            k=np.random.randint(54, 60)
        self.board=self.generate_board(k)
        # resetting entries and labels in board
        for cell in self.my_entries:
            cell.place_forget()
        self.my_entries=[]
        # creating new entries and labels
        for i in range(9):
            for j in range(9):
                if self.board[i][j]==0:
                    self.entry=Entry(self.frames[i//3][j//3], borderwidth=1, relief="ridge", font="Helvetica 15 bold", state="normal",
    ...
    #change number in cell without backspace
    ...
    while i<k-3:
        ...
        for j in range(1,10):
            ...
            if is_safe(board, coord[0], coord[1], j):
                board[coord]=j
                if solve_like_human(board,do_solve=False):
                    i=i-1
...
# finding whether there is only one number which is safe to place in the cell
...
# looking for rows, columns and boxes where there is only one empty cell
# function fills that cell
...
# functions solving sudoku like a human
# when it's impossible it solves with solve() function
def solve_like_human(board, do_solve=True):
    if do_solve==False:
        board=np.matrix.copy(board)
    try:
        while 0 in board:
